 ...
 */
void SciTEWin::SetStatusBarText(const char *s, int noPart) {
	GUI::gui_string barText = GUI::StringFromUTF8(s);
	if (noPart < 0 || noPart > SBR_NUMPARTS)
		noPart = 0;
	::SendMessage(reinterpret_cast<HWND>(wStatusBar.GetID()),
	              SB_SETTEXT, noPart, reinterpret_cast<LPARAM>(barText.c_str()));
}
...
//!-start-[ExtendedContextMenu]
			...
			subMenu[0].Add();
			subMenu[0].Add(localiser.Text("New").c_str(), IDM_NEW, true);
			subMenu[0].Add();
			...
			subMenu[0].Add(localiser.Text("Save a Copy").c_str(), IDM_SAVEACOPY, !filePath.IsUntitled());
			subMenu[0].Add();
			
			//!-[use user.tabcontext.menu.*]
			...
			//!-[use command.tabcontext.itemID.*=1]
			for (int item = 0; item < toolMax; item++) {
				int itemID = IDM_TOOLS + item;
				SString prefix = "command.name." + SString(item) + ".";
				SString commandName = props.GetNewExpand(prefix.c_str(), filePath.AsUTF8().c_str());
				prefix = "command.tabcontext." + SString(item) + ".";
				int intab = props.GetNewExpand(prefix.c_str(), FileNameExt().AsUTF8().c_str()).value();
				if (commandName.length() && intab) {
					GUI::gui_string lcommandName = localiser.Text(commandName.c_str());
					subMenu[0].Add(lcommandName.c_str(), itemID, true);
					isAdded = true;
				}
			}
...
//!-end-[ExtendedContextMenu]
	...
	case NM_CLICK:
		...
		if (notification->nmhdr.idFrom == IDM_STATUSWIN) {
			...
			//!-[added OnStatusBarClick begin]
			if (extender) {
				bool result = extender->OnStatusBarClick(pNMMouse->dwItemSpec,""); //should add text
				if (result) {
					//UpdateStatusBar(false);
					return;
				}
			}
			//!-[added OnStatusBarClick end]
			/*switch (pNMMouse->dwItemSpec) {
			case (SBR_NUMPARTS-1)://Display of status
				sbNum++;
...
void SciTEWin::Creation() {
	...
	//!-[change wStatusBar parts]
	int widths[] = {200,290,400,600,-1};
	::SendMessage(reinterpret_cast<HWND>(wStatusBar.GetID()),
	              SB_SETPARTS, SBR_NUMPARTS,
