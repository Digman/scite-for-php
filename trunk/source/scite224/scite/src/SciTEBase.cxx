...
const char *contributors[] = {
			"Kim Long http://scite-for-php.googlecode.com",
			"Atsuo Ishimoto",
			"Mark Hammond",
			"Francois Le Coguiec",
			"Dale Nagata",
...
void SciTEBase::UpdateStatusBar(bool bUpdateSlowData) {
	if (sbVisible) {
		...
		//!-[add code.page.name begin]
		const char* codePageName ;
		int  editorUnicodeMode  = CurrentBuffer()->unicodeMode + IDM_ENCODING_DEFAULT;
		switch (editorUnicodeMode) {
			case IDM_ENCODING_UCS2BE:
				codePageName = "UCS-2 BE";
				break;
			case IDM_ENCODING_UCS2LE:
				codePageName = "UCS-2 LE";
				break;
			case IDM_ENCODING_UTF8:
				codePageName = "UTF-8 BOM";
				break;
			...
			default:
				char sc[5];
				sprintf(sc,"%d",(int)wEditor.Call(SCI_GETCODEPAGE));
				codePageName = sc;
				break;
		}
		propsStatus.Set("code.page.name", codePageName);//!-[added code.page.name]
		propsStatus.SetInteger("CodePage", wEditor.Call(SCI_GETCODEPAGE));//!-[added CodePage]
		//!-[add code.page.name end]

		char sbKey[32];
		//!-[add statusbar parts]
		SString label;
		for (int i = 0; i < SBR_NUMPARTS ; i++) {
			sprintf(sbKey, "statusbar.text.%d", i+1);
			label = propsStatus.GetExpanded(sbKey);
			if (label.size()) {
				SetStatusBarText(label.c_str(),i);
			}
		}
		//!-[add statusbar parts]
		/* //!-[Remove]
		...
		if (msg.size() && sbValue != msg) {	// To avoid flickering, update only if needed
			SetStatusBarText(msg.c_str(),SBR_NUMPARTS-1);
			sbValue = msg;
		}*/
	} else {
		sbValue = "";
...
void SciTEBase::AddCommand(const SString &cmd, const SString &dir, JobSubsystem jobType, const SString &input, int flags) {
	...
	if (cmd.length()) {
		...
		if (jobType == jobCLI && !(flags & jobNoOutputPane)) //!-changed-[command.quiet.mode]
...
//!-end-[fix_invalid_codepage]
		UpdateStatusBar(false);//!-[add UpdateStatusBar]
		break;
