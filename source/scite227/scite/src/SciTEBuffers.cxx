...
void SciTEBase::New() {
	...
	//!-begin-[added new.buffer.in.utf8]
	if (props.GetInt("new.buffer.in.utf8")) {
		CurrentBuffer()->unicodeMode = uniCookie;//!-[init unicodeMode to UTF8]
	}//!-end-[added new.buffer.in.utf8]
...
void SciTEBase::ToolsMenu(int item) {
	...
	if (command.length()) {
		...
		bool filter = false;
		int quiet = 0; //!-changed-[command.quiet]
		int repSel = 0;
		...
		if (saveBefore == 2 || (saveBefore == 1 && (!(CurrentBuffer()->isDirty) || Save())) || SaveIfUnsure() != IDCANCEL) {
			...
			//!-begin changed-[command.quiet]
			propName = "command.quiet.";
			propName += itemSuffix;
			if (props.GetWild(propName.c_str(), FileNameExt().AsUTF8().c_str()).length())
				quiet = props.GetNewExpand(propName.c_str(), FileNameExt().AsUTF8().c_str()).value();
			switch (quiet) {
				case 1:
					flags |= jobQuiet;
					break;
				...
				case 3:
					flags |= jobQuiet;
					flags |= jobNoOutputPane;
					break;
				default:
					break;
			}
			//!-end changed-[command.quiet]
