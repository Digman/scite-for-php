...
//!-end-[utf8.auto.check]
		...
		wOutput.Call(SCI_SETCODEPAGE, codePage); //!-[added Output set codePage]
...
bool SciTEBase::SaveBuffer(FilePath saveName) {
	...
	if (props.GetInt("save.convert.indent")) {
		int tabSize = props.GetInt("tabsize");
		int useTabs = props.GetInt("use.tabs",1);
		SString useTabsChars = props.GetNewExpand("use.tabs.", ExtensionFileName().c_str());
		if (useTabsChars.length() != 0)
			useTabs = useTabsChars.value();
