...
void SciTEWin::Notify(SCNotification *notification) {
	...
	case NM_RCLICK:
		...
		if (notification->nmhdr.idFrom == IDM_TABWIN) {
			...
			AddToPopUp("Save a Copy", IDM_SAVEACOPY, !filePath.IsUntitled());
			AddToPopUp("");
			AddToPopUp("Copy File Full Path", IDM_COPYPATH, true);
			AddToPopUp("Copy File Directory", IDM_COPYFILEDIR, true);
			AddToPopUp("Copy File Name With Ext", IDM_COPYFILENAMEEXT, !filePath.IsUntitled());
			AddToPopUp("Copy File Name", IDM_COPYFILENAME, !filePath.IsUntitled());
			AddToPopUp("Copy File Ext", IDM_COPYFILEEXT, !filePath.IsUntitled());
